...
@vectorized(input=pd.DataFrame)
def process_tmp(df):
    # Process the text data in batches using the spaCy model
    # Assuming 'text' is the column name containing the text data to be processed
    processed_data = df['text'].apply(lambda x: ','.join(list(set([span.label_ for span in nlp(x).spans['sc']]))))
    
    return processed_data

#@vectorized(input=pd.DataFrame) 
#def process(df): logger.info('Number of rows:' + str(len(df))) 
...
def process(df):
    # Assuming 'text' is the column name containing the text data to be processed
    # and 'nlp' is the loaded spaCy model.
    # Adjust the column name and model loading logic as necessary.
    
    # Process the text data in batches using the spaCy model
    processed_data = df['text'].apply(lambda x: ','.join([str(doc[span.start:span.end]) + ':' + str(span.label_) for span in nlp(x).spans['INTELLECTUAL_DISABILITY']]))
